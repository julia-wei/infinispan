...
/**
 * A {@link CompletionService} that uses a supplied
 * {@link DistributedExecutorService} to execute tasks. This class arranges that
 * submitted tasks are, upon completion, placed on a queue accessible using
 * <tt>take</tt>. The class is lightweight enough to be suitable for transient
 * use when processing groups of tasks.
 * <p>
 ...
 * of a non serializable RunnableFuture object. Since a ExecutionService may
 * only be used with serializable request objects, this class must be used
 * instead.
 */
public class DistributedExecutionCompletionService<V> implements CompletionService<V> {
     ...
     * provide their own QueueingListener to possibly listen in on futures being
     * finished
     * 
     * @param executor
     *            the executor to use
     ...
     *            dedicated for use by this service
     * @param listener
     *            the listener to notify. To work properly this listner should
     *            at minimum call the super.futureDone or else this completion
     ...
     * internally this class sets the listener to provide ability to add to the
     * queue.
     */
    @Override
     ...
     * @param <K>
     */
    public <K> Future<V> submit(Callable<V> task, K... inputKeys) {
        if (task == null)
            throw new NullPointerException();
        NotifyingFuture<V> f = (NotifyingFuture<V>) executor.submit(task, inputKeys);
        f.attachListener(listener);
        return f;
    }
     ...
     * {@inheritDoc CompletionService}
     * <p>
     * This future may safely be used as a NotifyingFuture if desired. This is
     * because if it tries to set a listener it will be called immediately since
