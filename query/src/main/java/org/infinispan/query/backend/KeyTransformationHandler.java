...
/**
 * This transforms arbitrary keys to a String which can be used by Lucene as a
 * document identifier, and vice versa.
 * <p/>
 * There are 2 approaches to doing so; one for SimpleKeys: Java primitives (and
 * their object wrappers) and Strings, and one for custom, user-defined types
 * that could be used as keys.
 * <p/>
 * For SimpleKeys, users don't need to do anything, these keys are automatically
 * transformed by this class.
 * <p/>
 * For user-defined keys, two options are supported. Types annotated with
 * @Transformable, and declaring an appropriate
 * {@link org.infinispan.query.Transformer} implementation; and types for which
 * a {@link org.infinispan.query.Transformer} has been explicitly registered
 * through KeyTransformationHandler.registerTransformer().
 * 
...
public class KeyTransformationHandler {
    ...
    /**
     * Retrieves a {@link org.infinispan.query.Transformer} instance for this
     * key. If the key is not {@link org.infinispan.query.Transformable} and no
     * transformer has been registered for the key's class, null is returned.
     * 
     * @param keyClass
     *            key class to analyze
     * @return a Transformer for this key, or null if the key type is not
     *         properly annotated.
     */
    ...
    /**
     * Registers a {@link org.infinispan.query.Transformer} for the supplied key
     * class.
     * 
     * @param keyClass
     *            the key class for which the supplied transformerClass should
     *            be used
     ...
     * 
     * @param cache
     *            the cache for which we want to get the
     *            KeyTransformationHandler instance
