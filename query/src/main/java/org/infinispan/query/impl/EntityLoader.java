...
public class EntityLoader {
    ...
    public Object[] loadWithScores(EntityInfo entityInfo) {
        Object cacheKey = keyTransformationHandler.stringToKey(entityInfo.getId().toString(), cache.getClassLoader());
        Float score = (Float) entityInfo.getProjection()[0];
        return new Object[] { cache.get(cacheKey), score };
    }
    ...
    public List<Object[]> loadWithScores(EntityInfo... entityInfos) {
        int size = entityInfos.length;
        ArrayList<Object[]> list = new ArrayList<Object[]>(size);
        for (EntityInfo e : entityInfos) {
            list.add(loadWithScores(e));
        }
        return list;
    }
}
