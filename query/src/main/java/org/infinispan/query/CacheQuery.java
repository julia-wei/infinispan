 ...
 * its affiliates and other contributors as indicated by the @author tags. All
 * rights reserved. See the copyright.txt in the distribution for a full listing
 * of individual contributors. This is free software; you can redistribute it
 ...
 * {@link SearchManagerImpl}. This object can have methods such as list,
 * setFirstResult,setMaxResults, setFetchSize, getResultSize and setSort.
 * <p/>
 * 
...
public interface CacheQuery extends Iterable {
    ...
    /**
     * Returns the results of a search as a list with projection values.
     * 
     * @return list of objects that were found from the search.
     */
    List<Object[]> listWithScores();
     ...
     * @return a QueryResultIterator which can be used to iterate through the
     *         results that were found.
     */
    QueryIterator iterator(int fetchSize);

    /**
     * Returns the results of a search as a {@link QueryIterator}. This calls
     * {@link CacheQuery#iterator(int fetchSize)} but uses a default fetchSize
     * of 1.
     * 
     * @return a QueryResultIterator which can be used to iterate through the
     *         results that were found.
    ...
    /**
     * Lazily loads the results from the Query as a {@link QueryIterator} with a
     * given integer parameter - the fetchSize.
     * 
     * @param fetchSize
     *            integer to be passed into the lazy implementation of
     *            {@link QueryIterator}
     * @return a QueryResultIterator which can be used to <B>lazily</B> iterate
     *         through results.
     */
    QueryIterator lazyIterator(int fetchSize);

    /**
     * Calls the {@link CacheQuery#lazyIterator(int fetchSize)} method but
     * passes in a default 1 as a parameter.
     * 
     * @return a QueryResultIterator which can be used to <B>lazily</B> iterate
     *         through results.
     */
    ...
    /**
     * Sets the maximum number of results to the number passed in as a
     * parameter.
     * 
     * @param numResults
    ...
    /**
     * Return the Lucene {@link org.apache.lucene.search.Explanation} object
     * describing the score computation for the matching object/document in the
     * current query
     * 
     * @param documentId
    ...
    /**
     * Allows lucene to sort the results. Integers are sorted in descending
     * order.
     * 
     * @param s
     *            - lucene sort object
     ...
     * Each field is converted back to it's object representation, an Object[]
     * being returned for each "row"
     * <p/>
     * A projectable field must be stored in the Lucene index and use a
     * {@link org.hibernate.search.bridge.TwoWayFieldBridge} Unless notified in
     * their JavaDoc, all built-in bridges are two-way. All @DocumentId fields
     * are projectable by design.
     * <p/>
     * If the projected field is not a projectable field, null is returned in
     * the object[]
     * 
     * @param fields
